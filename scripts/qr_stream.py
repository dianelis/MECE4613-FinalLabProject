...
def main():
    ...
    # in a for loop, watch all the atomic files, robot_stream.jpg and 
    # decode and report if those files contain a qr code
    while cv2.waitKey(1) not in [ord('q'), ESC_KEY]:
        if watcher.has_changed():
            img = cv2.imread(IMG_PATH)
            data = decode_qrcode(img)
            # NOTE: If the image contains a qr code, 'data' will have it as a string,
            # otherwise, it'll be s null string ('')
